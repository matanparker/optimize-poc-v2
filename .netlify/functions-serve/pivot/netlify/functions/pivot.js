...
var require_utils = __commonJS({
  "netlify/functions/utils.js"(exports2, module2) {
    ...
    function filterByWindow2(data, dateColumn, days) {
    ...
    module2.exports = {
      ...
      filterByWindow: filterByWindow2,
      computeMetrics,
      mockUsers,
      mockFAQs,
      mockRules
...
// netlify/functions/pivot.js
var { createResponse, loadCSVData, filterByWindow } = require_utils();
exports.handler = async (event, context) => {
  if (event.httpMethod === "OPTIONS") {
    return createResponse({});
  }
  try {
    const window = parseInt(event.queryStringParameters?.window || "30");
    const { mediumData } = loadCSVData();
    const filteredData = filterByWindow(mediumData, "order_date", window);
    const groups = {};
    filteredData.forEach((row) => {
      const region = row.region || row.Location || "Unknown";
      const category = row.product_category || row.ProductCategory || "Unknown";
      const revenue = parseFloat(row.final_amount) || parseFloat(row.Revenue) || 0;
      const key = `${region}|${category}`;
      groups[key] = (groups[key] || 0) + revenue;
    });
    const rows = Object.entries(groups).map(([key, revenue]) => {
    ...
    return createResponse({ rows, simulated: true });
  } catch (error) {
