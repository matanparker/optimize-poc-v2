...
// netlify/functions/recommendations-apply.js
var { createResponse } = require_utils();
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (!ids || !Array.isArray(ids)) {
      return createResponse({ error: "Invalid request: ids array required" }, 400);
    }
    return createResponse({
      applied: ids,
      message: `Applied ${ids.length} recommendations successfully`,
      simulated: true
    });
  } catch (error) {
    console.error("Error in recommendations-apply function:", error);
    return createResponse({ error: "Invalid request body" }, 400);
  }
};
//# sourceMappingURL=recommendations-apply.js.map
