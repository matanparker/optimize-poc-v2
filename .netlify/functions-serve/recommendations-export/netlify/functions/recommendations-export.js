...
// netlify/functions/recommendations-export.js
var { createResponse, loadCSVData, computeMetrics } = require_utils();
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const metrics = computeMetrics(mediumData);
    let csvContent = "Metric,Value\n";
    csvContent += `Impressions,${metrics.impressions}
`;
    csvContent += `Reach,${metrics.reach}
`;
    csvContent += `Frequency,${metrics.frequency}
`;
    csvContent += `Conversions,${metrics.conversions}
`;
    csvContent += `Revenue,${metrics.revenue}
`;
    csvContent += `Cost,${metrics.cost}
`;
    csvContent += `CPC,${metrics.cpc}
`;
    csvContent += `Conversion Rate,${metrics.conversion_rate}
`;
    csvContent += `ROAS,${metrics.roas}
`;
    return {
      statusCode: 200,
      headers: {
        "Content-Type": "text/csv",
        "Content-Disposition": 'attachment; filename="presentation_data.csv"',
        "Access-Control-Allow-Origin": "*"
      },
      body: csvContent
    };
  } catch (error) {
    console.error("Error in recommendations-export function:", error);
    return createResponse({ error: "Internal server error" }, 500);
  }
};
//# sourceMappingURL=recommendations-export.js.map
